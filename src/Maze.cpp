...
Maze::Maze(unsigned int _w, unsigned int _h)
{
    m_width = _w;
    m_height = _h;
...
Maze::Maze(const std::string& _filename)
{
    ...
    for (unsigned int y = 0; y < m_height; ++y)
    {
        for (unsigned int x = 0; x < m_width; ++x)
        {
            ...
            if (img.getColour(x, y) == ngl::Vec4::white())
            {
...
void Maze::createDefaultObjects()
{
...
void Maze::keyEvent(int _x, int _y)
{
    std::cout << _x << ' ' << _y << '\n';
...
void Maze::update(float _dt)
{
    ...
    if (dir.x !=0  && getItem(pos.x + dir.x, pos.y + dir.y) == ItemType::Fruit)
    {
        newPos.x = pos.x + dir.x;
    }

    if (dir.y != 0 && getItem(pos.x + dir.x, pos.y + dir.y) == ItemType::Fruit)
    {
        newPos.y = pos.y + dir.y;
    }
    p->setPos(newPos);
    setItem(pos.x, pos.y, ItemType::Fruit, m_objects[Fruit]);
    setItem(newPos.x, newPos.y, ItemType::Player, m_objects[Player1]);



}
void Maze::setItem(unsigned int _x, unsigned int _y, ItemType _type, GameObject* _obj)
{
    size_t index = _y * m_width + _x;
...
void Maze::drawWall(unsigned int _x, unsigned int _y) const
{
